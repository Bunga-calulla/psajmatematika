...
window.getModuleSimulator = window.getModuleSimulator || function(id){ return ''; };

window.getModuleSimulator = function(id){
  if(id !== 'trigonometri') return window._moduleSimulatorFallback(id);
...
window.bindSimulator = function(id){
  if(id !== 'trigonometri') return window._bindSimulatorFallback(id);

  const $ = id => document.getElementById(id);
  const fmt = n => Number(n).toFixed(2).replace('.', ',');
  const fmt3 = n => Number(n).toFixed(3).replace('.', ',');
  const rad = deg => deg * Math.PI / 180;
  const mark = () => { if(window.Progress) Progress.setCoba(id); };
  ...
  function calcBuilding(){
    ...
    $('trigBuildingDistOut').textContent=d+' m'; $('trigBuildingAngleOut').textContent=a+'°'; $('trigEyeOut').textContent=fmt(e)+' m';
    $('trigBuildingResult').innerHTML=`<b>Tinggi bagian yang diukur:</b> ${fmt(h)} m<br><b>Tinggi gedung:</b> ${fmt(total)} m<br><small>Rumus: tan(${a}°) = tinggi ÷ ${d}</small>`;
    $('trigBuildingVisual').style.height=Math.min(245,70+h*6)+'px'; $('trigSightline').style.transform=`rotate(${-Math.min(55,a)}deg)`;
  }
  bDist.oninput=bAngle.oninput=eye.oninput=()=>{calcBuilding();mark()}; calcBuilding();

  // 2. Tangga
  const ll=$('trigLadderLen'), la=$('trigLadderAngle');
  function calcLadder(){
    const l=+ll.value, a=+la.value, h=l*Math.sin(rad(a)), base=l*Math.cos(rad(a));
    $('trigLadderLenOut').textContent=l+' m'; $('trigLadderAngleOut').textContent=a+'°'; $('trigLadderHeight').textContent=fmt(h)+' m'; $('trigLadderBase').textContent=fmt(base)+' m';
    $('trigLadderFormula').innerHTML=`<b>sin ${a}° = tinggi / ${l}</b> → tinggi = ${fmt(h)} m<br><b>cos ${a}° = jarak / ${l}</b> → jarak = ${fmt(base)} m`;
    $('trigLadderVisual').style.transform=`rotate(${-a}deg)`; $('trigLadderAngleVisual').textContent=a+'°';
  }
  ll.oninput=la.oninput=()=>{calcLadder();mark()}; calcLadder();
  ...
  function calcRoad(){
    ...
    $('trigRoadRiseOut').textContent=rise+' m'; $('trigRoadRunOut').textContent=horizontal+' m'; $('trigRoadRiseLabel').textContent=rise+' m'; $('trigRoadRunLabel').textContent=horizontal+' m';
    $('trigRoadResult').innerHTML=`<b>Sudut kemiringan:</b> ${fmt(a)}°<br><b>Grade:</b> ${fmt(percent)}%<br><small>tan θ = ${rise} ÷ ${horizontal}</small>`;
    $('trigRoadVisual').style.transform=`rotate(${-Math.max(2,Math.min(18,a))}deg)`;
  }
  rr.oninput=run.oninput=()=>{calcRoad();mark()}; calcRoad();

  // 4. Eksplorasi sudut
  const ae=$('trigAngleExplore');
  function calcExplore(){
    const a=+ae.value, s=Math.sin(rad(a)), c=Math.cos(rad(a)), t=Math.tan(rad(a));
    $('trigAngleExploreOut').textContent=a+'°'; $('trigSinValue').textContent=fmt3(s); $('trigCosValue').textContent=fmt3(c); $('trigTanValue').textContent=fmt3(t);
    $('trigExploreExplanation').innerHTML=`Untuk sudut <b>${a}°</b>: <b>sin θ = depan/miring</b>, <b>cos θ = samping/miring</b>, dan <b>tan θ = depan/samping</b>.`;
    $('trigExploreTriangle').style.transform=`rotate(${a-35}deg)`;
  }
  ae.oninput=()=>{calcExplore();mark()}; calcExplore();

  // 5. Graph
  const canvas=$('trigGraphCanvas'), ctx=canvas.getContext('2d'); let graph='sin';
  function drawGraph(){
    const w=canvas.width,h=canvas.height; ctx.clearRect(0,0,w,h); ctx.font='16px Arial';
    ...
    for(let i=0;i<=w;i++){
      const deg=i/w*360, r=rad(deg); let yv=graph==='sin'?Math.sin(r):graph==='cos'?Math.cos(r):Math.tan(r);
      if(graph==='tan' && Math.abs(yv)>4){drawing=false;continue;}
      const y=h/2-yv*(h/2.5); if(!drawing){ctx.beginPath();ctx.moveTo(i,y);drawing=true}else ctx.lineTo(i,y);
    }
    ctx.stroke();
    ctx.fillStyle='#555'; ctx.fillText('0°',8,h-10);ctx.fillText('90°',w*.25-15,h-10);ctx.fillText('180°',w*.5-20,h-10);ctx.fillText('270°',w*.75-20,h-10);ctx.fillText('360°',w-42,h-10);
    $('trigGraphInfo').innerHTML=graph==='tan'?'<b>tan x</b> → periode 180° dan tidak terdefinisi saat 90°, 270°, dan seterusnya.':'<b>'+graph+' x</b> → periode 360°, nilai berada antara −1 dan 1.';
  }
  document.querySelectorAll('[data-trig-graph]').forEach(btn=>btn.onclick=()=>{document.querySelectorAll('[data-trig-graph]').forEach(x=>x.classList.remove('active'));btn.classList.add('active');graph=btn.dataset.trigGraph;drawGraph();mark()}); drawGraph();

  // 6. Identity
  document.querySelectorAll('#trigIdentityOptions button').forEach(btn=>btn.onclick=()=>{
    const ok=btn.dataset.answer==='1'; $('trigIdentityFeedback').innerHTML=ok?'🎉 <b>Benar!</b> Identitas dasarnya adalah sin² θ + cos² θ = 1.':'Belum tepat. Ingat identitas dasar: <b>sin² θ + cos² θ = 1</b>.'; mark();
  });

  // 7. Special angles
  const special={30:{sin:'1/2',cos:'√3/2',tan:'√3/3'},45:{sin:'√2/2',cos:'√2/2',tan:'1'},60:{sin:'√3/2',cos:'1/2',tan:'√3'}};
  document.querySelectorAll('[data-special-angle]').forEach(btn=>btn.onclick=()=>{const a=btn.dataset.specialAngle,v=special[a];$('trigSpecialResult').innerHTML=`<b>${a}°</b> → sin = ${v.sin} · cos = ${v.cos} · tan = ${v.tan}`;mark()});

  // 8. Equation sin x = k
  $('trigSolveBtn').onclick=()=>{
    let k=Number($('trigEqK').value); if(!Number.isFinite(k)) k=0.5; k=Math.max(-1,Math.min(1,k)); $('trigEqK').value=k;
    const a=Math.asin(k)*180/Math.PI; let sol=[];
    if(Math.abs(k-1)<1e-9) sol=[90]; else if(Math.abs(k+1)<1e-9) sol=[270]; else if(Math.abs(k)<1e-9) sol=[0,180,360]; else {let x=a<0?360+a:a; let y=180-x; sol=[x,y].filter(v=>v>=0&&v<=360).sort((p,q)=>p-q)}
    $('trigEquationResult').innerHTML=`Untuk <b>sin x = ${k}</b> pada 0° ≤ x ≤ 360°, solusi: <b>${sol.map(v=>fmt(v)+'°').join(' dan ')}</b>.`; mark();
  };
  ...
  function renderMission(){const m=missions[mi];$('trigMissionBox').innerHTML=`<p>${m.q}</p><div class="trig-mission-input"><input id="trigMissionAnswer" type="number" step="0.01" placeholder="Masukkan jawaban"><span>${m.unit}</span></div>`;$('trigMissionProgress').textContent=`Misi ${mi+1} dari ${missions.length}`;$('trigMissionFeedback').textContent='';$('trigMissionNext').disabled=true;checked=false}
  $('trigMissionCheck').onclick=()=>{const m=missions[mi], input=$('trigMissionAnswer'), val=Number(input.value); if(!Number.isFinite(val)){ $('trigMissionFeedback').textContent='Masukkan jawaban terlebih dahulu.';return;} const correct=Math.abs(val-m.answer)<=Math.max(.15,Math.abs(m.answer)*.03); $('trigMissionFeedback').innerHTML=correct?`🎉 <b>Benar!</b> ${m.tip}`:`Belum tepat. ${m.tip}`; if(correct){$('trigMissionNext').disabled=false;checked=true;} mark()};
  $('trigMissionNext').onclick=()=>{if(!checked)return;mi++;if(mi>=missions.length){$('trigMissionBox').innerHTML='<div class="mission-finished">🏆 <b>Semua misi selesai!</b><br>Kamu sudah mencoba beberapa penerapan trigonometri di kehidupan nyata.</div>';$('trigMissionProgress').textContent='5 dari 5 selesai';$('trigMissionNext').disabled=true;$('trigMissionCheck').disabled=true;}else renderMission();mark()}; renderMission();
};